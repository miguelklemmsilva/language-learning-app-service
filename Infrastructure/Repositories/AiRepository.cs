...
public class AiRepository(HttpClient httpClient, TextTranslationClient textTranslationClient) : IAiRepository
{
    private string LocalizedSystemPrompt(string language, string region)
    {
        return language switch
        {
            ...
            "Portuguese" =>
                $"Você é um assistente útil que gera frases de exemplo. Quando uma palavra for fornecida, escreva uma frase curta de exemplo usando essa palavra. Use português de {region switch
                {
                    "Brazil" => "Brasil",
                    "Portugal" => "Portugal",
                    _ => "Portugal"
            ...
            "French" =>
                $"Vous êtes un assistant utile qui génère des phrases d'exemple. Lorsqu'un mot vous est fourni, écrivez une courte phrase d'exemple en utilisant ce mot. Utilisez le français de {region switch
                {
                    "France" => "France",
                    "Canada" => "Canada",
                    _ => "France"
    ...
    public async Task<TranslatedTextItem?> TranslateSentenceAsync(string sentence, string sourceLanguage)
    {
        ...
        Response<IReadOnlyList<TranslatedTextItem>> response =
            await textTranslationClient.TranslateAsync(options);
